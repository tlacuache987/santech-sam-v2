...
public final class ContentUrlRewritingFilter extends ZuulFilter {
	...
	public ContentUrlRewritingFilter(final String regex, final String replacement,
			final Collection<MediaType> supportedTypes) {
	...
	private static void rewriteContent(final RequestContext context, final String regex, final String replacement)
			throws Exception {
		assert context != null;
		if (context.getResponseBody() != null) {
			...
			try (final InputStream responseDataStream = context.getResponseDataStream()) {
				// FIXME What about character encoding of the stream?
				// It depends on the response content-type?
